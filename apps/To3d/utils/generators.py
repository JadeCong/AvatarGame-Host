...
class UnchunkedGenerator:
    ...
    # just only 243 frams
    def cur_2d_pose(self,cur_2dpoint,inputs):

        cur_inputs = np.zeros([1,244,17,2])
        cur_inputs[:,:243,:,:] = inputs
        cur_inputs[:,243,:,:] = cur_2dpoint
        ...
        if self.augment:
            ...
            cur_inputs[1, :, :, 0] *= -1
            cur_inputs[1, :, self.kps_left + self.kps_right] = cur_inputs[1, :, self.kps_right + self.kps_left]
...
# data = np.load('../data/yp.npy')
#
# cur_2dpoints = np.reshape(data[0],[1,17,2])
# inputs = cur_2dpoints.repeat(243,axis=0)
#
# for i in range(200):
#     cur_points = data[1]
#     outputs = cur_2d_pose(cur_points,inputs)
#     inputs = outputs[0]
