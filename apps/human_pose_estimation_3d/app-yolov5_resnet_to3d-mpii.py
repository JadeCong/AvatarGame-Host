...
sys.path.append("../../nns/resnet")
import resnet
import resnet.models as pose_models
from resnet.config import cfg
from resnet.config import update_config
from resnet.core.inference_new import get_final_preds
from resnet.utils.transforms import get_affine_transform
...
def get_pose_estimation_prediction(config, pose_model, image, centers, scales, transform, device):
    """Predict the 2D human pose using the resnet model
    ...
    for center, scale in zip(centers, scales):
        trans = get_affine_transform(center, scale, rotation, config.MODEL.RESNET.IMAGE_SIZE)
        # Crop smaller image of people
        model_input = cv2.warpAffine(
            image,
            trans,
            (int(config.MODEL.RESNET.IMAGE_SIZE[0]), int(config.MODEL.RESNET.IMAGE_SIZE[1])),
...
def mainFunc(args):
    ...
    model_resnet = eval('pose_models.'+config.MODEL.NAME.RESNET+'.get_pose_net')(config, is_train=False)
    if config.EVAL.RESNET.MODEL_FILE:
        print('=> loading model from {}'.format(config.EVAL.RESNET.MODEL_FILE))
        model_resnet.load_state_dict(torch.load(config.EVAL.RESNET.MODEL_FILE), strict=False)
    else:
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        pred_pose_2d = get_pose_estimation_prediction(config, model_resnet, im0s[0], centers, scales, transform=pose_transform, device=device)
        ...
        if config.EVAL.RESNET.USE_FILTERS_2D:
            ...
            filters_2d = constructFilters(pred_pose_2d.shape, freq=1, mincutoff=1, beta=0.01)
