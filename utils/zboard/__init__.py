...
def toId(klid):
    lid = klid & (2 ** 16 - 1)
    lid_hex = hex(lid)
    val=lid_hex.split('x')[1]
    noz=8-len(val)
    lang=(noz*'0')+val
    return lang

def refreshSymbols():
    _os_keyboard.refresh_name_tables()

def changeLocale(newLang):
    global _oldLang
    if _oldLang==0:
        _oldLang=GetKeyboardLayout()
        print(_oldLang)
    if newLang==0:
        return
    #print('Change In Language Detected!!!')
    layouts=list(GetKeyboardLayoutList())
    nli=layouts.index(newLang)
    oli=layouts.index(_oldLang)
    steps=nli-oli
    while(steps!=0):
        if steps>0:
            hkl=user32.ActivateKeyboardLayout(1,0x00000100)
            steps-=1
        elif steps<0:
            hkl=user32.ActivateKeyboardLayout(0,0x00000100)
            steps+=1
    refreshSymbols()
    _oldLang=newLang
