...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            print("Human Detection Time: {}, Human Detection FPS: {}".format(detect_time, detect_fps))  # for test

            # Stream results
            if view_img:
                # cv2.imshow(p, im0)
                text = "FPS:{0:0>5.2f}/{1:0>6.2f}ms".format(detect_fps, detect_time*1000)
                cv2.putText(im0, text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2, cv2.LINE_AA)  # for test
                cv2.namedWindow(str(p), cv2.WINDOW_NORMAL)  # for test
                cv2.imshow(str(p), im0)  # for test
